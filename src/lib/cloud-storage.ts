 ...
 */
export async function saveToCloud(cloudName: string, rooms: Room[], utilityCosts: UtilityCostSet[]): Promise<string> {
  try {
 ...
 */
export async function loadFromCloud(cloudName: string): Promise<CloudExportData | null> {
  try {
    // Use a consistent filename based on cloud name
    const filename = `${cloudName}.json`;
    ...
    if (!fileExists || fileExists.length === 0 || !fileExists.some(file => file.name === filename)) {
      toast({
